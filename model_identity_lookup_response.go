/*
Connect API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package trinsic_api

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the IdentityLookupResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &IdentityLookupResponse{}

// IdentityLookupResponse struct for IdentityLookupResponse
type IdentityLookupResponse struct {
	// Whether the given phone number is known to have an identity in the network.
	IdentityInNetwork bool `json:"identityInNetwork"`
}

type _IdentityLookupResponse IdentityLookupResponse

// NewIdentityLookupResponse instantiates a new IdentityLookupResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewIdentityLookupResponse(identityInNetwork bool) *IdentityLookupResponse {
	this := IdentityLookupResponse{}
	this.IdentityInNetwork = identityInNetwork
	return &this
}

// NewIdentityLookupResponseWithDefaults instantiates a new IdentityLookupResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewIdentityLookupResponseWithDefaults() *IdentityLookupResponse {
	this := IdentityLookupResponse{}
	return &this
}

// GetIdentityInNetwork returns the IdentityInNetwork field value
func (o *IdentityLookupResponse) GetIdentityInNetwork() bool {
	if o == nil {
		var ret bool
		return ret
	}

	return o.IdentityInNetwork
}

// GetIdentityInNetworkOk returns a tuple with the IdentityInNetwork field value
// and a boolean to check if the value has been set.
func (o *IdentityLookupResponse) GetIdentityInNetworkOk() (*bool, bool) {
	if o == nil {
		return nil, false
	}
	return &o.IdentityInNetwork, true
}

// SetIdentityInNetwork sets field value
func (o *IdentityLookupResponse) SetIdentityInNetwork(v bool) {
	o.IdentityInNetwork = v
}

func (o IdentityLookupResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o IdentityLookupResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["identityInNetwork"] = o.IdentityInNetwork
	return toSerialize, nil
}

func (o *IdentityLookupResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"identityInNetwork",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varIdentityLookupResponse := _IdentityLookupResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varIdentityLookupResponse)

	if err != nil {
		return err
	}

	*o = IdentityLookupResponse(varIdentityLookupResponse)

	return err
}

type NullableIdentityLookupResponse struct {
	value *IdentityLookupResponse
	isSet bool
}

func (v NullableIdentityLookupResponse) Get() *IdentityLookupResponse {
	return v.value
}

func (v *NullableIdentityLookupResponse) Set(val *IdentityLookupResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableIdentityLookupResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableIdentityLookupResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableIdentityLookupResponse(val *IdentityLookupResponse) *NullableIdentityLookupResponse {
	return &NullableIdentityLookupResponse{value: val, isSet: true}
}

func (v NullableIdentityLookupResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableIdentityLookupResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


